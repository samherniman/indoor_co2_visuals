...
There were measurements in `r length(unique(buildings_wide_df_short$countryname))` seperate countries. `r new_countries_sentence` The following graph shows the distribution of the most common building types in the month of `r lubridate::today()  |> lubridate::month(label = TRUE, abbr = FALSE)`.
...
Here's a histogram showing how many measurements have been recorded each week since the start of the project. Over the last 12 months there have been `r measurements_this_year` building measurements which is `r measurements_this_year/12 |> round()` per month or `r measurements_this_year/52 |> round()` per week. 
