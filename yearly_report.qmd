...
start_date <- (lubridate::ymd("2025-12-30"))
```
...
```{r}
world_sf <- rnaturalearth::ne_countries() |> sf::st_make_valid()
sf::sf_use_s2(FALSE)

buildings_snapped <- sf::st_join(buildings_wide_df_short, world_sf)
transit_snapped <- sf::st_join(transit_long_df_short, world_sf)
```
...
buildings_wide_df_short_geo <- 
  # buildings_wide_df_short_geo |> 
  readRDS(here::here("data/derivative/buildings_wide_df_short_geo_yr.rds")) |> 
  dplyr::rowwise() |> 
  dplyr::mutate(
...
transit_long_df_short_geo <- 
  # transit_long_df_short_geo |> 
  readRDS(here::here("data/derivative/transit_long_df_short_geo_yr.rds")) |> 
  dplyr::rowwise() |> 
  dplyr::mutate(
...
countries_count <- 
buildings_snapped |> 
    dplyr::select(uid = obs_number, name_en, type.x) |> 
    sf::st_drop_geometry() 

# not yet done. need to give transit data a country col
countries_count_transit <- 
    transit_snapped |> 
    dplyr::group_by(uid, name_en, type.x) |> 
    dplyr::count() |> 
    dplyr::select(-n) |> 
...
countries_count <- 
  ...
  # dplyr::left_join(iso_3166_a2, by = dplyr::join_by(name_en == code))  
  

# countries_count <- countries_count[order(countries_count$n, decreasing = TRUE),] |> 
#   tidyr::drop_na()
# levels(countries_count$type.x)
countries_count <- 
    countries_count |>
      dplyr::mutate(
        # type.x = forcats::fct_relevel(type.x, "building", "transit") #|> forcats::fct_rev()
        # name_en = forcats::fct_infreq(name_en) |> forcats::fct_rev()
        ) |> 
      dplyr::group_by(name_en, type.x) |> 
      dplyr::count() 
# levels(countries_count$type.x)

countries_labels <- 
  countries_count |> 
  tidyr::pivot_wider(names_from = type.x, values_from = n) |>
  dplyr::mutate(
  ...
  tidyr::pivot_longer(!name_en, names_to = "type.x", values_to = "label_n") |> 
  tidyr::drop_na()
...
countries_count <- 
  dplyr::left_join(
    countries_count,
    countries_labels,
    by = dplyr::join_by(name_en, type.x)
  ) 
...
#   countries_count |> 
#     dplyr::mutate(
#         # iso_name = forcats::fct_infreq(iso_name) |> forcats::fct_rev(),
#         type.x = forcats::fct_relevel(
#           type.x, 
#           "transit", 
#           "building"
#           )
...
countries_count |> 
  tidyr::drop_na() |> 
  dplyr::filter(n >= 100) |> 
    ggplot2::ggplot(aes(x = forcats::fct_reorder(name_en, n), y = n, fill = forcats::fct_rev(type.x))) +
    geom_col() +
...
# countries_count <- 
  ...
  dplyr::left_join(
    world_pop_df,
    by = dplyr::join_by(name_en)
  ) |> 
  dplyr::mutate(
    obs_per_person = (n/pop_est) * 1000000,
    people_per_obs = (pop_est/n) 
  ) |> 
  tidyr::drop_na() |> 
  dplyr::filter(n >= 100) |> 
    ggplot2::ggplot(aes(x = forcats::fct_reorder(name_en, obs_per_person), y = obs_per_person
    # fill = forcats::fct_rev(type.x)
    )) +
    geom_col() +
    # ggrepel::geom_text_repel(
    geom_text(
      aes(y = obs_per_person, label = round(obs_per_person)), 
      vjust = -0.5, 
      colour = "black"
      # min.segment.length = 1000,
      # direction = "y",
    ...
    labs(x = "Country", y = "Observations per 1 million people") +
    tidyplots_theme() +
    theme(
      legend.title = element_blank(),
      axis.text.x = element_text(
