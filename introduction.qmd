...
start_date <- (lubridate::ymd("2025-11-30"))
```
...
This month there were `r nrow(buildings_wide_df_short)` measurements of `r length(unique(buildings_wide_df_short$combined_id))` unique buildings around the world. The median CO~2~ measurement was `r median(mean_vals$ppmavg, na.rm = TRUE) |> round()` ppm. This graph shows you the distribution of all the CO~2~ measurements this month. The dashed red line shows the median. @cr-histco2plot 
