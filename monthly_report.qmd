...
start_date <- lubridate::ymd("2026-02-28")
```
...
Measuring CO~2~ inside is a cheap way of measuring the air quality in indoor environments. When we breathe, we exhale  CO~2~ and it gets trapped inside the room we are in. If the building has good ventilation it will leave quickly. If it has bad ventilation, it stays in the room and builds up.

If there is bad ventilation, then smoke from cooking can build up and that's bad for you. Same thing for VOCs from perfumes, as well as gas leaks, radon, and mold spores. At high concentrations in artificial environments, they contribute to all sorts of things: cancer [@guzman_air_2025], Alzheimer's [@fu_air_2020; @shi_long-term_2020; @olloquequi_inhalation_2024], Parkinson's [@shi_long-term_2020], childhood asthma [@akar-ghibril_indoor_2020; @rosser_air_2022; @pan_interactions_2020; @breysse_indoor_2010; @hulin_indoor_2010], childhood lung problems [@maung_indoor_2022; @kurmi_indoor_2012], and heart conditions [@uzoigwe_emerging_2013].
...
There were measurements in `r length(unique(countries_count$best_country_name))` separate countries. `r new_countries_sentence`  
