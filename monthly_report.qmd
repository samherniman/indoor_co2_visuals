...
start_date <- (lubridate::ymd("2025-12-31"))
```
...
countries_count <- 
buildings_wide_df_short |> 
    dplyr::select(uid = obs_number, country_code, type.x) |> 
    sf::st_drop_geometry() 

# not yet done. need to give transit data a country col
countries_count_transit <- 
    transit_long_df_short |> 
    dplyr::group_by(uid, country_code, type.x) |> 
    dplyr::count() |> 
    dplyr::select(-n) |> 
...
countries_count <- 
    countries_count |>
      ...
      dplyr::group_by(iso_name, type.x) |> 
      dplyr::count() 
# levels(countries_count$type.x)
...
countries_labels <- 
  ...
  tidyr::pivot_longer(!iso_name, names_to = "type.x", values_to = "label_n") |> 
  tidyr::drop_na()
...
countries_count <- 
  dplyr::left_join(
    countries_count,
    countries_labels,
    by = dplyr::join_by(iso_name, type.x)
  ) 
# countries_count <- 
#   countries_count |> 
#     dplyr::mutate(
...
countries_count |> 
  tidyr::drop_na() |> 
  dplyr::filter(n > 1) |> 
    ggplot2::ggplot(aes(x = iso_name, y = n, fill = forcats::fct_rev(type.x))) +
    geom_col() +
